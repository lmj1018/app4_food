...
export class Roulette extends EventTarget {
  ...
  public start() {
    ...
    if (this._autoRecording) {
      try {
        this._recorder.start().catch(() => {
        });
      } catch (error) {
        console.warn('[roulette] failed to start recorder, fallback to direct start', error);
      }
    }
  }
  ...
  public setMarbles(names: string[]) {
    ...
    const spawn = this._stage?.spawn;
    const spawnColumns = Math.max(1, spawn?.columns ?? 10);
    const spawnSpacingX = Math.max(0.1, spawn?.spacingX ?? 0.6);
    const spawnX = spawn?.x ?? 10.25;
    const spawnY = spawn?.y ?? 0;
    const spawnVisibleRows = Math.max(1, spawn?.visibleRows ?? 5);
    ...
    if (totalCount > 0) {
      const cols = Math.min(totalCount, spawnColumns);
      const rows = Math.ceil(totalCount / spawnColumns);
      const lineDelta = -Math.max(0, Math.ceil(rows - spawnVisibleRows));
      const centerX = spawnX + (cols - 1) * spawnSpacingX * 0.5;
      const centerY = spawnY + (1 + rows) / 2 + lineDelta;

      const spawnWidth = Math.max((cols - 1) * spawnSpacingX, 1);
