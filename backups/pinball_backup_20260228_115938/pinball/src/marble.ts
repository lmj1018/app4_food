...
import type { StageSpawnDef } from './data/stageDef';
...
export class Marble {
  ...
  constructor(
    ...
    const columns = Math.max(1, spawn?.columns ?? 10);
    const spacingX = Math.max(0.1, spawn?.spacingX ?? 0.6);
    const spawnX = spawn?.x ?? 10.25;
    const spawnY = spawn?.y ?? 0;
    const visibleRows = spawn?.visibleRows ?? 5;
    const maxLine = Math.ceil(max / columns);
    const line = Math.floor(order / columns);
    const lineDelta = -Math.max(0, Math.ceil(maxLine - visibleRows));
    ...
    physics.createMarble(
      order,
      spawnX + (order % columns) * spacingX,
      spawnY + maxLine - line + lineDelta,
    );
  }
